...
That schema describes completed Kiro Security Power scan output. It requires generated fields such as `scanId`, `findingId`, `occurrenceId`, fingerprints,
...
Use the schema only as an optional compatibility source when the user supplies an existing `kiro-security-power.findings` JSON artifact. In that case, extract the available fields into the triage normalization record and preserve the original IDs as source identifiers. The triage result contract is defined in `../references/triage-result-contract.md`.
...
the current Kiro project's attached GitHub repository, or the current local repository's GitHub remote.

If the user asks to pull from GitHub without typing an `owner/repo` or URL, first infer the GitHub repository from the current Kiro project attachment when that metadata is available. Prefer that attached repository over a local path or local git remote. If no Kiro project attachment is visible, fall back to the current repository's GitHub remote. Only ask for a repository URL or `owner/repo`
...
SARIF results, CVE/GHSA or advisory descriptions, scanner tickets, bug bounty report snippets, Jira/Linear issue URLs or searches, Kiro Security Power finding artifacts, or a freeform vulnerability claim. If useful, ask for the repository path or affected file/component at the same time.
...
- input id, scanner id, SARIF rule/result id, CVE/GHSA id, ticket id, or Kiro Security Power `findingId`/`occurrenceId` when present
- title or short claim
- source type: `sarif`, `cve`, `advisory`, `scanner_ticket`,
  `bug_bounty`, `kiro_security_finding`, `freeform`, or `unknown`
...
3. Normalize each supplied or imported finding into a triage item.
   ...
   - Do not invent scanner fields, generated Kiro Security Power ids, severity, or remediation just to satisfy another schema.
...
13. Return a concise Markdown summary; provide the raw result only when the user asks for it.
14. When the workflow opened a Kiro triage intake, persist the completed result through `security_submit_triage_assessment`; otherwise return the concise Markdown summary and show fenced JSON only on the user's request.
...
The `triage-finding/v0` contract does not define a dedicated owner field. Do not add undocumented fields to the Kiro result payload. Put owner-hint text in existing Markdown output, evidence, or recommended-next-step text when it is useful.

## Kiro Output Contract
...
The Kiro result payload, when the standalone workflow uses `security_submit_triage_assessment`, must include:
...
Prefer the Kiro chat result over showing raw JSON. The intended default UX is:

1. generate the valid `triage-finding/v0` result internally
2. persist it only through the bounded standalone triage tool when an intake exists
3. respond with the concise Markdown summary

Use the fenced JSON block only when the user explicitly asks to see or copy the raw result contract.
