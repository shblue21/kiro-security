---
name: track-findings
description: Track validated Kiro Security Power findings in Linear, Jira, GitHub issues, or draft GitHub security advisories. Use it for one finding or an explicitly selected batch of up to 25 findings tracked as Linear, Jira, or GitHub issues. Includes duplicate checks, exact previews, approval-gated writes, and readback. Do not use it for scans or fixes.
---
# Track Findings

## Objective

Track findings from one sealed Kiro Security Power scan as Linear issues, Jira issues, GitHub issues, or one draft GitHub security advisory. Do not change the scan bundle. Use one provider and one destination per run. Show the exact payload and get approval before writing.
...
If a batch stops before completion, reconstruct completed work from provider readback, rerun source and duplicate checks, and preview the remaining items again. Do not continue from conversation memory alone.
