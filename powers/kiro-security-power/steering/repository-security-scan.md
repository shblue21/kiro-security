...
Kiro starts scans from chat only. Before substantive work, call `security_get_capabilities`, inspect the live native tool surface for `invoke_sub_agent` with `general-task-execution`, then call `security_start_scan` with `mode: "standard"`. Retain the one-time coordinator lease token and generation only in this top-level coordinator, following `POWER.md`; a busy lease makes the returned scan read-only until acquired. Immediately load `security_get_scan_context` and treat its immutable target, scope, SECURITY.md guidance path, rank input, artifact root, and canonical outputs as authoritative.

Run the bundled Kiro capability preflight with profile `security_scan` using verified runtime facts. Continue only on a ready result. There is no app setup flow, dashboard start flow, or fallback scanner. If native delegation is unavailable, preserve the running scan and report the exact limitation; do not claim exhaustive coverage.
...
After the chat-only start has returned a `scanId`, its authoritative context is loaded, and the `security_scan` preflight is ready, create a Kiro goal for the scan if the runtime exposes goal tools and no active goal already covers this scan. The objective should state that the scan must not stop until the resolved files in scope have been covered and the required coverage artifacts prove that closure.

Use objective wording shaped like:

`Run the Kiro Security Power repository/scoped-path scan for <resolved target>; do not stop until every in-scope file/worklist row has a completion receipt or explicit deferred closure, every candidate has required ledger receipts, and the final report is written.`
...
Start this plan only after the chat-only start loaded the authoritative scan context and the `security_scan` capability preflight returned `ready`.
...
6. Run `$validation` as the third step, for each candidate that came out of discovery and each open, reportable, or deferred seeded/root-control ledger row that still needs closure.
  ...
  - As coverage-ledger rows are validated, keep the saved per-finding validation reports current enough that reportable, suppressed, not_applicable, and deferred closure rows survive a stopped turn or later phase summarization, including exact root-control file:line and seed-anchor file:line when distinct.
...
- In large repository-wide scans, checkpoint the ranked runtime-surface worklist and initial coverage ledger to disk before deep sink review or validation. A run that stops after frontier mapping should still leave auditable coverage artifacts.
