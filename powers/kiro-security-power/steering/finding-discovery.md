...
description: Use when Kiro is already in the finding-discovery phase of a security scan or the user explicitly asks to discover candidate security findings in a repository or code change. Do not use as the primary trigger for full PR, commit, branch, patch, or repository scans.
---
...
- Read `../references/scan-artifacts-and-ledger.md`.
- Generate `rank_input.jsonl` with the Engine-provided direct worklist helper or `<python_command> -m kiro_security.codex_contract.generate_rank_input make-diff-rank-input`; preserve its exact rows.
- Copy every diff row with the Engine-provided direct worklist helper or `<python_command> -m kiro_security.codex_contract.generate_rank_input copy-deep-review-input`. Diff scans do not rank or drop changed files before deep review.
...
- When the diff is too large to review credibly as one parent-agent pass, use file-review subagents when they are available under the resolved scan authorization and follow the shared scoped deep-review rules in `../references/scan-artifacts-and-ledger.md#scoped-deep-review`.

### Exhaustive Repository Or Scoped-Path Workflow

If the scan target is repository-wide or a scoped path, follow the procedure in `../references/repository-wide-scan.md` and every required reference it lists.
