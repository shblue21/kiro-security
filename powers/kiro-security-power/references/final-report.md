# Final Report and Kiro Review Directives

Use this guidance when authoring canonical report semantics and returning the generated Kiro Security Power report and review directives.
...
When `security_complete_scan` is available, use it with the current top-level coordinator lease token and generation to complete the scan. Never place that token in a report, artifact, provenance record, log, event, or subagent prompt. In Kiro CLI or another terminal/chat host without that tool, run `python -m kiro_security.codex_contract.finalize_scan_contract --scan-dir <scan_dir> --source-root <repo_root>` after writing the completed canonical JSON. Do not mark the scan goal complete until this command succeeds and the generated markdown report exists.
...
## Kiro Review Directives

For Kiro app rendering, emit one `::code-comment{...}` directive per surviving finding in the final response. The markdown report and review directives should agree on title, file, line range, and core explanation.

Map the final report severity to Kiro directive priority only when emitting the directive:
...
For each reportable finding, emit a Kiro review directive in this form:
...
- `title` should include the mapped Kiro directive priority, formatted like `[P1] Example title`
- `file` should be an absolute path
- `start` and `end` should be tight 1-based line numbers
- `priority` should match the mapped Kiro directive priority
