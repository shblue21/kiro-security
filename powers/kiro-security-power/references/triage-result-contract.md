...
That schema is for completed Kiro Security Power scan artifacts. It requires scan metadata, generated finding ids, occurrence ids, fingerprints, severity,
...
If the user supplies a valid `kiro-security-power.findings` artifact, use it as an input source and map its fields into this triage contract. Preserve `findingId`, `occurrenceId`, and `ruleId` as source identifiers; do not require new triage inputs to conform to the completed-scan schema.
