...
# upstream relative path: scripts/workbench_target.py
# upstream sha256: c6cec7207af2091bccd4677ddc8ea8b21d601830d9e9d7003ec4ffbfcd0f5989
...
def kiro_workspace_content_digest(target: Path, state_root: Path) -> str:
    """Use the upstream worktree digest with only the Kiro state root excluded."""
    require_clean_submodule_worktrees(target)
    repository, pathspec = git_worktree_context(target)
    try:
        state_relative = state_root.resolve(strict=True).relative_to(repository).as_posix()
    except (OSError, ValueError) as exc:
        raise SystemExit("Kiro workspace state root must stay inside the Git worktree.") from exc
    include_pathspec = ":(top,glob)**" if pathspec == "." else f":(top,glob){pathspec}/**"
    pathspecs = [include_pathspec, f":(exclude,top,glob){state_relative}/**"]
    tracked = git_bytes(
        repository,
        "diff",
        "--binary",
    ...
    update_digest_field(digest, b"format", b"kiro-security-snapshot/v1")
    ...
    for raw_path in sorted(path for path in untracked.split(b"\0") if path):
        ...
        update_digest_field(digest, b"untracked-mode", str(stat.S_IMODE(metadata.st_mode)).encode())
        if stat.S_ISLNK(metadata.st_mode):
            update_digest_field(digest, b"untracked-kind", b"symlink")
            update_digest_field(digest, b"untracked-content", os.fsencode(os.readlink(path)))
            ...
            update_digest_field(digest, b"untracked-size", str(content_size).encode())
            update_digest_field(digest, b"untracked-content-sha256", content_digest.digest())
        else:
            raise SystemExit(f"Unsupported untracked file type: {relative_path}")
    return f"kiro-security-snapshot/v1:sha256:{digest.hexdigest()}"
...
def directory_content_digest(target: Path, *, excluded: tuple[Path, ...] = ()) -> str:
    ...
    digest = hashlib.sha256()
    update_digest_field(digest, b"format", b"kiro-security-directory/v1")
    for path in paths:
