...
# upstream relative path: scripts/finalize_scan_contract.py
# upstream sha256: c072bc5b441079095a272abe62d39e8f4dcfb961a406c4ae64eef940bfdef26b
...
#!/usr/bin/env python3
"""Validate and seal additive Kiro Security Power scan-contract artifacts."""
...
SCHEMA_VERSION = "1.0"
FINGERPRINT_ALGORITHM = "kiro-security/v1"
...
def _generate_report_projection(
    ...
    spec = importlib.util.spec_from_file_location("kiro_security_report_projection", script)
...
def _windows_scan_local_files() -> Any:
    ...
    if _WINDOWS_SCAN_LOCAL_FILES is None:
        ...
        spec = importlib.util.spec_from_file_location("kiro_security_windows_scan_files", script)
        if spec is None or spec.loader is None:
...
def _enrich_findings(manifest: dict[str, Any], findings: dict[str, Any]) -> None:
    ...
    for index, finding in enumerate(_require_list(findings, "findings", "findings")):
        ...
        expected_finding_id = _stable_id("kspf", fingerprint)
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    if manifest.get("documentType") != "kiro-security-power.scan-manifest":
        raise ContractError("manifest.documentType: expected kiro-security-power.scan-manifest")
...
def _validate_findings(manifest: dict[str, Any], findings: dict[str, Any]) -> None:
    if findings.get("documentType") != "kiro-security-power.findings":
        raise ContractError("findings.documentType: expected kiro-security-power.findings")
...
def _sarif_result(
    ...
    partial_fingerprints = {
        "kiroSecurity/v1": finding["fingerprints"]["primary"],
    }
...
def build_sarif(
    ...
    run: dict[str, Any] = {
        ...
        "properties": {
            "kiroSecuritySchemaVersion": manifest["schemaVersion"],
            "kiroSecurityTargetKind": target["kind"],
        },
    }
...
def write_sarif_projection(
    ...
    scan_dir = _require_scan_directory(scan_dir)
    schema_dir = schema_dir or Path(__file__).resolve().parents[2] / "schemas"
