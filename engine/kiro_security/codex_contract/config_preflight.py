...
# upstream relative path: scripts/config_preflight.py
# upstream sha256: cff0180424086e5cb5b289bdf3f5bf106d5c360050bc154324e2d72bb6dcc84f
# allowed Kiro changes: package-local imports, Kiro names/prefixes, workspace artifact root, worker cap 4, Deep round cap 5, Kiro runtime fact provider.
# prohibited semantic changes: none; keep the upstream algorithm and contract intact.

#!/usr/bin/env python3
"""Evaluate Kiro Security Power capability profiles against the active Kiro runtime."""
...
DEFAULT_KIRO_HOME = Path(os.environ.get("KIRO_HOME", Path.home() / ".kiro"))
SYSTEM_CONFIG = Path("/etc/kiro/config.toml")
DEFAULT_CONFIG = DEFAULT_KIRO_HOME / "config.toml"
VALID_SEVERITIES = {"block", "warn", "suggest"}
VALID_MULTI_AGENT_OWNERS = {"native", "kiro-bridge"}
...
def parse_args() -> argparse.Namespace:
    ...
    config_input.add_argument(
        ...
        action="append",
        help=(
            "Kiro config.toml layer, from lower to higher precedence. "
    ...
    parser.add_argument(
        "--kiro-config-profile",
        help="Selected Kiro config profile name, when the session uses one.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--multi-agent-worker-slots",
        type=positive_int,
        help="Usable delegated-worker slots observed from the active Kiro runtime.",
    )
    parser.add_argument(
...
def project_config_paths(project_root: Path, cwd: Path) -> list[Path]:
    ...
    return [directory / ".kiro" / "config.toml" for directory in directories]
...
def config_profile_layer_path(profile: str | None) -> Path | None:
    ...
    path = DEFAULT_KIRO_HOME / f"{profile}.config.toml"
...
def resolve_multi_agent_context(
    *,
    ...
    runtime_worker_slots: int | None,
    runtime_provenance: str | None,
) -> dict[str, Any]:
    runtime_facts_supplied = any(
        value is not None
        for value in (runtime_owner, runtime_version, runtime_session_cap, runtime_worker_slots)
    )
    ...
    if runtime_owner == "kiro-bridge" and runtime_provenance != "verified-bridge":
        raise ValueError(
            "kiro-bridge ownership requires --multi-agent-runtime-provenance verified-bridge"
        )
    ...
    elif runtime_owner == "kiro-bridge":
        version = "v2"
    ...
    if owner == "kiro-bridge" and version != "v2":
        raise ValueError("kiro-bridge ownership requires multi-agent runtime version v2")
    if runtime_session_cap is not None and version != "v2":
        raise ValueError("--multi-agent-session-cap is valid only for a V2 runtime")
    if (
        runtime_session_cap is not None
        and runtime_worker_slots is not None
        and runtime_worker_slots != runtime_session_cap - 1
    ):
        raise ValueError(
            "conflicting runtime capacity facts: --multi-agent-worker-slots must equal "
            "--multi-agent-session-cap minus the root thread"
        )
    ...
    if backend_found and owner != "kiro-bridge":
        raise ValueError(
            "backend_config.max_multiagent_concurrency does not prove bridge ownership; "
            "pass --multi-agent-runtime-owner kiro-bridge only when the active runtime "
    ...
    if owner != "kiro-bridge" and feature_found and enabled and agent_threads_found:
        raise ValueError("agents.max_threads cannot be set when multi_agent_v2 is enabled")

    if version == "v1":
        mode = "v1"
    elif version == "v2" and owner == "kiro-bridge":
        mode = "bridge-v2"
    ...
    return {
        ...
        "runtime_worker_slots": runtime_worker_slots,
...
def evaluate_multi_agent_capacity(
    ...
    *,
    ...
    runtime_worker_slots = multi_agent_context.get("runtime_worker_slots")
    if runtime_worker_slots is not None:
        path = "runtime.multi_agent.usable_worker_slots"
        found, actual, source = True, runtime_worker_slots, "runtime-fact"
        worker_slots = actual
    elif multi_agent_mode == "v1":
        ...
        elif multi_agent_context["owner"] == "kiro-bridge":
...
def evaluate_requirement(
    ...
    *,
    ...
    if capability["kind"] == "multi_agent_capacity":
        evaluated = evaluate_multi_agent_capacity(
            result,
            capability,
            config_layers=config_layers,
            effective_config=effective_config,
        ...
        if (
            requirement.get("runtime_fact_required")
            and evaluated.get("status") == "pass"
            and evaluated.get("source") != "runtime-fact"
        ):
            return {
                **evaluated,
                "status": "unknown",
                "check": "runtime_multi_agent_worker_slots",
                "observedSource": evaluated.get("source"),
            }
        return evaluated
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    cli_profile_selected = args.kiro_config_profile is not None
    if args.config:
        ...
        profile_layer_path = config_profile_layer_path(args.kiro_config_profile)
    ...
    for path in config_paths:
        ...
        if path in project_config_paths_set:
            # Kiro strips project-local profile selection and definitions before
    ...
    config_profile, embedded_config_profile = resolve_active_config_profile(
        config_layers=config_layers,
        override=args.kiro_config_profile,
    ...
    multi_agent_context = resolve_multi_agent_context(
        ...
        runtime_worker_slots=args.multi_agent_worker_slots,
