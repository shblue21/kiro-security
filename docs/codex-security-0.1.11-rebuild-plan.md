...
- 같은 UI가 auto-inclusion steering 전용 파일 `~/.kiro/steering/kiro-security-power.md`와 Hook 전용 파일 `~/.kiro/hooks/kiro-security-power.json`을 설치하고 현재 상태를 표시한다. 기존 Extension-managed 설치가 현재 Extension과 다르면 사용자에게 `Update & Reload` 승인을 받은 뒤 관리 대상 runtime·steering·Hook·MCP 항목·Permission 규칙만 갱신한다. unmanaged 충돌과 관련 없는 사용자 설정은 덮어쓰거나 제거하지 않는다. custom Agent와 Power import는 사용하지 않는다.
