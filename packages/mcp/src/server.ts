...
const toolDefinitions = [
  ...
  { name: "security_deep_submit_worker_result", description: "Submit one completed independent discovery worker with exhaustive reviewed-path receipts, worker threat model, and evidence-grounded candidates.", inputSchema: { type: "object", properties: { workspaceRoot: { type: "string" }, scanId: { type: "string" }, workerId: { type: "string" }, claimToken: { type: "string" }, reviewedPaths: { type: "array", items: { type: "string" } }, threatModel: { type: "string" }, summary: { type: "string" }, candidates: { type: "array", items: { type: "object" } } }, required: ["workspaceRoot", "scanId", "workerId", "claimToken", "reviewedPaths", "threatModel", "candidates"], additionalProperties: false } },
...
async function callTool(name: string, rawArguments: unknown): Promise<unknown> {
  ...
  switch (name) {
    ...
    case "security_deep_submit_worker_result": return engine.request("deep_submit_worker", { scanId: requiredString(params, "scanId", 256), workerId: requiredString(params, "workerId", 256), claimToken: requiredString(params, "claimToken", 256), reviewedPaths: params.reviewedPaths, threatModel: params.threatModel, summary: params.summary, candidates: params.candidates }, 120_000);
