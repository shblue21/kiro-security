...
4. Deep mode is model-orchestrated: use \`security_deep_get_status\`, run exactly six independent workers per round through \`security_deep_claim_worker\` and \`security_deep_submit_worker_result\`, then use the merge tools until a complete round adds zero new canonical candidates or round 10 is explicitly capped. Never substitute Standard results.
